...
namespace BigC_auth {
	...
	public:
		static std::string encrypt_string(const std::string& plain_text, const std::string& key, const std::string& iv) {
			...
			try {
				...
				MessageBoxA(0, ex.what(), "BigC", MB_ICONERROR);
				exit(0);
			}
			return cipher_text;
		}
		...
		static std::string decrypt_string(const std::string& cipher_text, const std::string& key, const std::string& iv) {
			...
			try {
				...
				MessageBoxA(0, c_xor("Invalid API/Encryption key"), "BigC", MB_ICONERROR);
				exit(0);
			}
			return plain_text;
		}
	...
	std::string api_endpoint = c_xor("https://webmat.store/api/handler.php");
	...
	public:
		...
		void init() {
			...
			auto post_data =
				...
				c_xor("&api_version=") + encryption::encrypt("1.2", api_key, init_iv) +
			...
			if (response == c_xor("program_doesnt_exist")) {
				MessageBoxA(0, c_xor("The program key you tried to use doesn't exist"), "BigC", MB_ICONERROR);
			...
			if (!decoded_response[c_xor("success")])
				MessageBoxA(0, std::string(decoded_response[c_xor("message")]).c_str(), "BigC", MB_ICONERROR);
		...
		bool login(std::string username, std::string password, std::string hwid = "default") {
			...
			if (!is_initialized) {
				MessageBoxA(0, c_xor("The program wasn't initialized"), "BigC", MB_ICONERROR);
		...
		std::vector<unsigned char> file(std::string file_name, std::string hwid = "default"){
			...
			if(!is_initialized){
				...
				return to_uc_vector(c_xor("not_initialized"));
			}

			if(!logged_in){
				MessageBoxA(0, c_xor("You can only grab server sided files after being logged in."), "BigC", MB_ICONERROR);

				return to_uc_vector(c_xor("not_logged_in"));
			}

			auto post_data =
				c_xor("file_name") + encryption::encrypt(file_name, api_key, session_iv) +
				c_xor("&sessid=") + encryption::hex_encode(session_id);

			auto response = do_request(c_xor("file"), post_data);
			...
			auto file = encryption::hex_decode(decoded_response[c_xor("response")]);
		...
		std::string var(std::string var_name, std::string hwid = "default") {
			...
			if (!logged_in) {
				MessageBoxA(0, c_xor("You can only grab server sided variables after being logged in."), "BigC", MB_ICONERROR);
	...
	private:
		...
		static std::string do_request(std::string type, std::string post_data) {
			...
			curl_easy_setopt(c_url, CURLOPT_NOPROXY, "webmat.store");

			curl_easy_setopt(c_url, CURLOPT_SSL_VERIFYPEER, 0);
			curl_easy_setopt(c_url, CURLOPT_SSL_VERIFYHOST, 0);

			curl_easy_setopt(c_url, CURLOPT_PINNEDPUBLICKEY, c_xor("sha256//0SbikzDpzS8cpdFgW6AhEzXnwAZoR3+udVLpW4/mbvQ="));
