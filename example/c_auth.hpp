...
namespace c_auth {
	...
	std::string api_endpoint = "https://webmat.store/api/handler.php";

	std::string user_agent = "Mozilla cAuth";
	...
	public:
		...
		void init() {
			...
			if (response == "program_doesnt_exist") {
				MessageBoxA(0, "The program key you tried to use doesn't exist", "cAuth", MB_ICONERROR);
			...
			if (!decoded_response["success"])
				MessageBoxA(0, std::string(decoded_response["message"]).c_str(), "cAuth", MB_ICONERROR);

			auto response_data = utils::split(decoded_response["response"], '|');
		...
		bool login(std::string username, std::string password, std::string hwid = "default") {
			...
			if (!is_initialized) {
				MessageBoxA(0, "The program wasn't initialized", "cAuth", MB_ICONERROR);
		...
		std::vector<unsigned char> file(std::string file_name, std::string hwid = "default"){
			...
			if(!logged_in){
				MessageBoxA(0, "You can only grab server sided files after being logged in.", "cAuth", MB_ICONERROR);
		...
		std::string var(std::string var_name, std::string hwid = "default") {
			...
			if (!logged_in) {
				MessageBoxA(0, "You can only grab server sided variables after being logged in.", "cAuth", MB_ICONERROR);
	...
	private:
		...
		static std::string do_request(std::string type, std::string post_data) {
			...
			curl_easy_setopt(c_url, CURLOPT_PINNEDPUBLICKEY, "sha256//Oz5IyuaxNeKvkYfJOEFgJoKiC6e6W1V2ajOECzss5bg=");
		...
		void load_user_data(nlohmann::json data) {
			...
			user_data.expires = utils::timet_to_tm(
				utils::string_to_timet(data["expires"])
			);
